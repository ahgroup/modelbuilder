...
modelpath = paste0(basepath,"/stratified_models")
modelfilename <- paste0(modelpath,"/simulate_",modelname,"_ode.R")
...
path = paste0(basepath,"/filled_tables/")
...
for (i in 1:length(par_vec))
{
  ...
  #args_list$w_Rle <- par_vec[i]  #substitute the value for the parameter we want to change
  args_list$nu_Sle <- par_vec[i]  #substitute the value for the parameter we want to change
  ...
  outcome[i] = tail(sim$ts$Sle,1) #final number of susceptible
}
