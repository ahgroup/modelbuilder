...
#' result <- simulate_SIRSd_model_discrete() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_SIRSd_model_discrete(S = 2000,I = 2,R = 0) 
...
simulate_SIRSd_model_discrete <- function(S = 1000, I = 1, R = 0, b = 0.002, g = 1, w = 1, n = 0, m = 0, tstart = 0, tfinal = 100, dt = 0.1) 
{ 
  #Function that encodes simulation loop 
  SIRSd_model_fct <- function(vars, pars, times) 
  {
    with( as.list(c(vars,pars)), {  
      ...
      {
        ts[ct,] = c(t,S,I,R) 
        Sp = S + dt*(-b*S*I +w*R +n -m*S) 
        Ip = I + dt*(+b*S*I -g*I -m*I) 
        Rp = R + dt*(+g*I -w*R -m*R) 
        S = Sp 
        I = Ip 
        R = Rp 
  ...
  #Running the model 
  simout <- SIRSd_model_fct(vars = varvec, pars = parvec, times = timevec) 
