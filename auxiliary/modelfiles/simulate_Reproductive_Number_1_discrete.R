...
#' result <- simulate_Reproductive_Number_1_discrete() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_Reproductive_Number_1_discrete(S = 2000,I = 2,R = 0,N = 0) 
...
simulate_Reproductive_Number_1_discrete <- function(S = 1000, I = 1, R = 0, N = 0, b = 0.002, g = 1, tstart = 0, tfinal = 100, dt = 0.1) 
{ 
  #Function that encodes simulation loop 
  Reproductive_Number_1_fct <- function(vars, pars, times) 
  {
    with( as.list(c(vars,pars)), {  
      ts = data.frame(cbind(times, matrix(0,nrow=length(times),ncol=length(vars)))) 
      colnames(ts) = c('time','S','I','R','N') 
  ...
  varvec = c(S = S, I = I, R = R, N = N) 
  parvec = c(b = b, g = g) 
  timevec = seq(tstart, tfinal,by = dt) 
  #Running the model 
  simout <- Reproductive_Number_1_fct(vars = varvec, pars = parvec, times = timevec) 
