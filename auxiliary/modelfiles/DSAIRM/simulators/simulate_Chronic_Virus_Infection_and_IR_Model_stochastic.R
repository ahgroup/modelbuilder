...
#' result <- simulate_Chronic_Virus_Infection_and_IR_Model_stochastic() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_Chronic_Virus_Infection_and_IR_Model_stochastic(U = 2e+05,I = 0,V = 2,F = 2,T = 2) 
...
simulate_Chronic_Virus_Infection_and_IR_Model_stochastic <- function(U = 1e+05, I = 0, V = 1, F = 1, T = 1, n = 0, dU = 0, dI = 1, dV = 2, b = 2e-05, p = 5, g = 1, rF = 0.001, dF = 1, kF = 0.01, rT = 0.001, dT = 1, kT = 0.01, tfinal = 100, rngseed = 123) 
{ 
  #Block of ODE equations for adaptivetau 
  Chronic_Virus_Infection_and_IR_Model_fct <- function(y, parms, t) 
  {
    with(as.list(c(y,parms)),   
     { 
       #specify each rate/transition/reaction that can happen in the system 
     rates = c(b*V*U, b*g*V*U, dF*F, dI*I, dT*T, dU*U, dV*V, kT*T*I, n, p*I/(1+kF*F), rF*I, rT*T*V)
     return(rates) 
      }
	 	)   
  ...
  simout = adaptivetau::ssa.adaptivetau(init.values = varvec, transitions = transitions,
                  	 	 	 rateFunc = Chronic_Virus_Infection_and_IR_Model_fct, params = parvec, tf = tfinal) 
