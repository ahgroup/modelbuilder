...
#' result <- simulate_Coronavirus_vaccine_model_stochastic() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_Coronavirus_vaccine_model_stochastic(S = 2000,P = 2,A = 2,I = 2,R = 0,H = 0,D = 0) 
...
simulate_Coronavirus_vaccine_model_stochastic <- function(S = 1000, P = 1, A = 1, I = 1, R = 0, H = 0, D = 0, thetaP = 0.001, thetaI = 0.001, thetaA = 5e-04, nu = 0.3, sigma = 0.2, gammaA = 0.125, gammaI = 0.1, gammaH = 0.2, phi = 0.05, rho = 0.01, n = 1003, nuInverse = 0.7, phiInverse = 0.95, rhoInverse = 0.99, c = 1, alpha = 1, tstart = 0, tfinal = 100, dt = 0.1, rngseed = 123) 
{ 
  #Block of ODE equations for adaptivetau 
  Coronavirus_vaccine_model_fct <- function(y, parms, t) 
  {
    with(as.list(c(y,parms)),   
     { 
       #specify each rate/transition/reaction that can happen in the system 
     rates = c(gammaA*A, sigma*P, (S/n)*c*alpha*thetaA*A, (S/n)*c*alpha*thetaI*I, (S/n)*c*alpha*thetaP*P, gammaH*H, gammaI*I, nu*(S/n)*c*alpha*thetaA*A, nu*(S/n)*c*alpha*thetaI*I, nu*(S/n)*c*alpha*thetaP*P, nuInverse*(S/n)*c*alpha*thetaA*A, nuInverse*(S/n)*c*alpha*thetaI*I, nuInverse*(S/n)*c*alpha*thetaP*P, phi*gammaI*I, phiInverse*gammaI*I, rho*gammaH*H, rhoInverse*gammaH*H)
     return(rates) 
      }
	 	)   
  ...
  simout = adaptivetau::ssa.adaptivetau(init.values = varvec, transitions = transitions,
                  	 	 	 rateFunc = Coronavirus_vaccine_model_fct, params = parvec, tf = tfinal) 
